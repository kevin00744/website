...
class ImportStaticWordPress extends Command
{
    ...
    {
        // WordPress 文章頁的常見特徵（含 Elementor 頁面）
        $selectors = [
            'article.post',
            'article.type-post',
            '.post-content',
        ...
        try {
            ...
            if (
                str_contains($bodyClass, 'single') ||
                str_contains($bodyClass, 'wp-singular') ||
                str_contains($bodyClass, 'elementor-page') ||
                preg_match('/\bpage-id-\d+\b/', $bodyClass)
            ) {
                return true;
            }
        } catch (\Exception $e) {}
